...
% Equations of motion
setPhase(phase)
tau = -k1*phi - k2*phid - k3*theta - k4*thetad; % lei de controle
...
%         final({phi == phif})
%         final({phid == 0})
%         final({theta^2 <= 0.01^2})
        final({thetad^2 <= 0.01^2})
        final({phi^2 <= 0.01^2})
        final({phid^2 <= 0.01^2})        
...
opt.k1 = subs(k1,solution); 
opt.k2 = subs(k2,solution); 
opt.k3 = subs(k3,solution); 
opt.k4 = subs(k4,solution); 
...
% filename = 'wheelie_controle_Px2.mat';
